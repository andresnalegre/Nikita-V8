## Nikita-V8
- Firmware identity is now `Nikita` (`device_info` firmware_origin, JS SDK vendor `"nikita"`, About screen, CLI greeting)
- New `nikita` CLI command: `info` (one parseable device snapshot), `init`, and `memory add/forget/clear`
- Agent memory moved onto the SD card at `/ext/nikita/memory.txt`, so the phone, the desktop app and the bridge share one set of remembered facts
- HID: new **PARASITE** mode in `USB Keyboard & Mouse` — the Flipper drives a host file explorer live over USB HID (arrows move, OK opens, Back goes up), driven by the phone's Remote Control over BLE; the host monitor is the return channel
- See [documentation/Nikita.md](/documentation/Nikita.md)
