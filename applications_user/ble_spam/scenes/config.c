...
void scene_config_on_enter(void* _ctx) {
    ...
    // Momentum's variable_item_list carries a header row (the attack title);
    // Nikita's Unleashed-based SDK does not export that call, so the title is
    // simply dropped here. Cosmetic only -- the config list is unchanged.
