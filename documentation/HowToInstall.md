...
> **Nikita firmware has no release channel of its own.** Everything below —
> the web installer, the GitHub releases, the update server — belongs to
> Unleashed upstream and installs *Unleashed*, not this fork. To install
> Nikita, build it yourself ([How to build](/documentation/HowToBuild.md)) and
> flash the resulting `dist/f7-C/flipper-z-f7-update-local` package with
> qFlipper or `./fbt flash_usb_full`. The pages below stay here because the
> flashing steps themselves are identical.
